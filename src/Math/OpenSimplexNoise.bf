...
namespace Voxis
{
    public class OpenSimplex2S
    {
        ...
        public this(int64 seed)
        {
            perm = new int16[PSIZE];
            permGrad2 = new Grad2[PSIZE];
            permGrad3 = new Grad3[PSIZE];
            permGrad4 = new Grad4[PSIZE];
            int16[] source = scope int16[PSIZE];
            for (int16 i = 0; i < PSIZE; i++)
                source[i] = i;
            for (int i = PSIZE - 1; i >= 0; i--)
            {
                int64 tempseed = seed * 6364136223846793005L + 1442695040888963407L;
		...
		public double Noise2Abs(double x, double y)
		{
			return (Noise2(x, y) + 1.0) * 0.5;
		}

		public double Noise2FBM(double x, double y, int octaves, double amplitude, double frequency, double lacunarity, double gain)
		{
			double result = 0.0;
			double localFreq = frequency;
			double localAmp = amplitude;

			for (int i = 0; i < octaves; i++)
			{
				result += localAmp * Noise2(localFreq * x, localFreq * y);
        ...
        private double noise4_Base(double xs, double ys, double zs, double ws)
        {
            ...
            // Point contributions
            for (LatticePoint4D c in LOOKUP_4D[index])
            {
		...
		static ~this()
		{
			...
			delete LOOKUP_2D;
			delete LOOKUP_3D;
			DeleteContainerAndItems!(LOOKUP_4D);
		}
        ...
        static this()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                LatticePoint3D c0 = scope LatticePoint3D(i1, j1, k1, 0);
                LatticePoint3D c1 = scope LatticePoint3D(i1 + i2, j1 + j2, k1 + k2, 1);

                // (1, 0, 0) vs (0, 1, 1) away from octant.
                LatticePoint3D c2 = scope LatticePoint3D(i1 ^ 1, j1, k1, 0);
                LatticePoint3D c3 = scope LatticePoint3D(i1, j1 ^ 1, k1 ^ 1, 0);

                // (1, 0, 0) vs (0, 1, 1) away from octant, on second half-lattice.
                LatticePoint3D c4 = scope LatticePoint3D(i1 + (i2 ^ 1), j1 + j2, k1 + k2, 1);
                LatticePoint3D c5 = scope LatticePoint3D(i1 + i2, j1 + (j2 ^ 1), k1 + (k2 ^ 1), 1);

                // (0, 1, 0) vs (1, 0, 1) away from octant.
                LatticePoint3D c6 = scope LatticePoint3D(i1, j1 ^ 1, k1, 0);
                LatticePoint3D c7 = scope LatticePoint3D(i1 ^ 1, j1, k1 ^ 1, 0);

                // (0, 1, 0) vs (1, 0, 1) away from octant, on second half-lattice.
                LatticePoint3D c8 = scope LatticePoint3D(i1 + i2, j1 + (j2 ^ 1), k1 + k2, 1);
                LatticePoint3D c9 = scope LatticePoint3D(i1 + (i2 ^ 1), j1 + j2, k1 + (k2 ^ 1), 1);

                // (0, 0, 1) vs (1, 1, 0) away from octant.
                LatticePoint3D cA = scope LatticePoint3D(i1, j1, k1 ^ 1, 0);
                LatticePoint3D cB = scope LatticePoint3D(i1 ^ 1, j1 ^ 1, k1, 0);

                // (0, 0, 1) vs (1, 1, 0) away from octant, on second half-lattice.
                LatticePoint3D cC = scope LatticePoint3D(i1 + i2, j1 + j2, k1 + (k2 ^ 1), 1);
                LatticePoint3D cD = scope LatticePoint3D(i1 + (i2 ^ 1), j1 + (j2 ^ 1), k1 + k2, 1);
            ...
            for (int i = 0; i < 256; i++)
            {
                LOOKUP_4D[i] = new LatticePoint4D[lookup4DPregen[i].Count];
                for (int j = 0; j < lookup4DPregen[i].Count; j++)
                {
                    LOOKUP_4D[i][j] = latticePoints[lookup4DPregen[i][j]];
                }
            }

			// latticePoints unused now, can delete
			DeleteContainerAndItems!(latticePoints);

			// lookup4DPregen unused now, can delete
			DeleteContainerAndItems!(lookup4DPregen);

            GRADIENTS_2D = new Grad2[PSIZE];
            Grad2[] grad2 = scope Grad2[](
            ...
            for (int i = 0; i < grad2.Count; i++)
            {
                grad2[i].dx /= N2; grad2[i].dy /= N2;
            }
            for (int i = 0; i < PSIZE; i++)
            {
                GRADIENTS_2D[i] = grad2[i % grad2.Count];
            }

            GRADIENTS_3D = new Grad3[PSIZE];
            Grad3[] grad3 = scope Grad3[](
            ...
            for (int i = 0; i < PSIZE; i++)
            {
                GRADIENTS_3D[i] = grad3[i % grad3.Count];
            }

            GRADIENTS_4D = new Grad4[PSIZE];
            Grad4[] grad4 = scope Grad4[](
            ...
            for (int i = 0; i < grad4.Count; i++)
            {
                grad4[i].dx /= N4; grad4[i].dy /= N4; grad4[i].dz /= N4; grad4[i].dw /= N4;
            }
            for (int i = 0; i < PSIZE; i++)
            {
                GRADIENTS_4D[i] = grad4[i % grad4.Count];
            }
        }
        ...
        private class LatticePoint3D
        {
            ...
            public this(int xrv, int yrv, int zrv, int lattice)
            {
        ...
        private class LatticePoint4D
        {
            ...
            public this(int xsv, int ysv, int zsv, int wsv)
            {
        ...
        {
            public double dx, dy;
            public this(double dx, double dy)
            {
        ...
        private struct Grad3
        {
            ...
            public this(double dx, double dy, double dz)
            {
        ...
        {
            public double dx, dy, dz, dw;
            public this(double dx, double dy, double dz, double dw)
            {
